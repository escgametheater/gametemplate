...
public sealed class GoogleUniversalAnalytics
{
    ...
    private Queue<byte[]> processQueue = new Queue<byte[]>();
    ...
    public bool sendHit()
    {
		...
		SendDataToGoogle(postData);
		
		// thread-safe dispatch...
		/*
		Monitor.Enter(processQueue);
		try {
			processQueue.Enqueue(postData);
		}
		finally {
    ...
    private void ThreadUpdate () 
    {
        while (this.threadStarted) {
			...
			if (Monitor.TryEnter(processQueue)) {
			    try {
					if (processQueue.Count > 0) {
						byte[] data = processQueue.Dequeue();
			...
			// Must apply delay for rate limiting messages
			Thread.Sleep(10);
        }
    }
	*/

    private void SendDataToGoogle (byte[] postData)
	{
		GameInterface.writeLog("SendDataToGoogle");
        ...
        {
			/*
            WWW request = new WWW(useHTTPS ? httpsCollectUrl : httpCollectUrl, postData);

            if (request.isDone) 
            {
                if (!String.IsNullOrEmpty(request.error)) 
                    GameInterface.writeLog("GA request error: " + request.text);
            }
			*/
        }
    }
